...
(function($) {
    var parseCells = function(rows) {
        var cells = [], numRows = rows.length, x, y, row, rowLength, cell;
        console.log(rows);
        for (y = 0; y < numRows; y++) {
            row = rows[y];
            cells[y] = [];
            for (x = 0,
            rowLength = row.length; x < rowLength; x++) {
                cell = row[x];
                if (cell !== " ") {
                    cells[y].push(cell)
                }
    ...
    var parseRects = function(cells) {
        var rects = [], numRows = cells.length, numCols = numRows === 0 ? 0 : cells[0].length, cell, height, width, x, y, rectX, rectY;
        cells = cells.slice();
        for (y = 0; y < numRows; y++) {
        ...
        for (y = 0; y < numRows; y++) {
            for (x = 0; x < numCols; x++) {
                ...
                if (cell !== Tiles.Template.SINGLE_CELL) {
                    while (width + x < numCols && cell === cells[y][x + width]) {
                        width++
                    }
                    while (height + y < numRows && cell === cells[y + height][x]) {
                        height++
                    }
                }
    ...
    Tiles.Template.prototype.append = function(other) {
        ...
        var startY = this.numRows, i, len, rect;
        for (i = 0,
        len = other.rects.length; i < len; i++) {
            rect = other.rects[i];
            this.rects.push(new Rectangle(rect.x,startY + rect.y,rect.width,rect.height))
        }
    ...
    Tiles.Template.fromJSON = function(rows) {
        ...
        return new Tiles.Template(rects,cells.length > 0 ? cells[0].length : 0,cells.length)
    }
    ;
    Tiles.Template.prototype.toJSON = function() {
        var LABELS = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", NUM_LABELS = LABELS.length, labelIndex = 0, rows = [], i, len, rect, x, y, label;
...
Tiles.UniformTemplates = {
    get: function(numCols, targetTiles) {
        var numRows = Math.ceil(targetTiles / numCols), rects = [], x, y;
        for (y = 0; y < numRows; y++) {
            for (x = 0; x < numCols; x++) {
...
(function($) {
    ...
    Grid.prototype.resizeColumns = function() {
        ...
        return Math.max(1, Math.floor((panelWidth + this.cellPadding) / (this.cellSizeMin + this.cellPadding)))
    }
    ;
    Grid.prototype.resizeCellSize = function() {
        var panelWidth = this.getContentWidth();
        return Math.ceil((panelWidth + this.cellPadding) / this.numCols) - this.cellPadding
    }
    ;
    ...
    Grid.prototype.updateTiles = function(newTileIds) {
        ...
        var numTiles = newTileIds.length, newTiles = [], i, tile, tileId, index;
    ...
    function uniques(items) {
        var results = [], numItems = items ? items.length : 0, i, item;
    ...
    Grid.prototype.addTiles = function(newTileIds, prepend) {
        ...
        var prevTileIds = [], prevTileCount = this.tiles.length, i;
        for (i = 0; i < prevTileCount; i++) {
            prevTileIds.push(this.tiles[i].id)
        }
        var tileIds = prepend ? newTileIds.concat(prevTileIds) : prevTileIds.concat(newTileIds);
    ...
    Grid.prototype.ensureTemplate = function(numTiles) {
        if (!this.template || this.template.numCols !== this.numCols) {
            ...
            if (missingRects > 0) {
                this.template.append(this.createTemplate(this.numCols, missingRects));
                this.isDirty = true
            }
        }
    }
    ...
    function wasOrWillBeVisible(viewRect, tile, newRect) {
        ...
        if (tile) {
            if (!(tile.top > viewMaxY || tile.top + tile.height < viewRect.y || tile.left > viewMaxX || tile.left + tile.width < viewRect.x)) {
                return true
            }
        }
        if (newRect) {
            if (!(newRect.y > viewMaxY || newRect.y + newRect.height < viewRect.y || newRect.x > viewMaxX || newRect.x + newRect.width < viewRect.x)) {
                return true
            }
        }
        return false
    ...
    Grid.prototype.shouldRedraw = function() {
        ...
        var shouldRedraw = this.isDirty || this.tilesAdded.length > 0 || this.tilesRemoved.length > 0;
        return shouldRedraw
    }
    ;
    Grid.prototype.redraw = function(animate, onComplete) {
        ...
        for (tileIndex = 0; tileIndex < numTiles; tileIndex += pageSize) {
            ...
            console.log(this);
            for (i = 0,
            len = priorityTiles.length; i < len; i++) {
                tile = priorityTiles[i];
                added = $.inArray(tile, this.tilesAdded) >= 0;
                cellRect = priorityRects[i];
                pixelRect = new Tiles.Rectangle(cellRect.x * cellPlusPadding,cellRect.y * cellPlusPadding,cellRect.width * cellPlusPadding - this.cellPadding,cellRect.height * cellPlusPadding - this.cellPadding);
                tile.resize(cellRect, pixelRect, animate && !added && wasOrWillBeVisible(viewRect, tile, pixelRect), duration);
                if (added) {
                    animateTile = animate && wasOrWillBeVisible(viewRect, null, pixelRect);
                    if (animateTile && this.getAppendDelay) {
                        appendDelay = this.getAppendDelay(cellRect, pageRects, priorityRects, tile, pageTiles, priorityTiles)
                    } else {
                        appendDelay = 0
                    }
        ...
        for (i = 0,
        len = this.tilesRemoved.length; i < len; i++) {
...
(function($) {
    ...
    $.fn.extend({
        wptiles: function(opts) {
                console.log(opts);
            var $el = $(this), $templates = $("#" + opts.id + "-templates"), $pagination = $("#" + opts.id + "-pagination"), grid, curr_large_template = false, using_small = false, get_first_grid = function() {
                var grid;
                ...
                var set_bylines_color = function($bylines, color) {
                    $bylines.css("color", color);
                    $bylines.find(":header").css("color", color)
                }
                  , $image_bylines = $(".wp-tiles-tile-with-image .wp-tiles-byline", $el);
                if ($image_bylines.get(0)) {
                    if ("random" !== opts.byline_color) {
                        $image_bylines.css("background-color", opts.byline_color)
                    }
            ...
            grid = $.extend(new Tiles.Grid($el), {
                ...
                templateFactory: {
                    get: function(numCols, numTiles) {
                        ...
                        while (missingRects > 0) {
                            var copyRects = [], i, t = get_template().copy();
                ...
                createTile: function(data) {
                    ...
                    if ($(".wp-tiles-tile-with-image", $el).get(0)) {
                        ...
                        $(".wp-tiles-tile-bg", $el).css({
                            ...
                            "-ms-filter": "progid:DXImageTransform.Microsoft.AlphaImageLoader( src='" + bg_img_src + "', sizingMethod='scale')",
                            filter: "progid:DXImageTransform.Microsoft.AlphaImageLoader( src='" + bg_img_src + "', sizingMethod='scale')"
                        });
                        $bg_img.first().remove()
                    }
                    return tile
                ...
                nextPage: function() {
                    ...
                    if (!opts.next_query.opts) {
                        ...
                        $.each(["hide_title", "link", "byline_template", "byline_template_textonly", "images_only", "image_size", "text_only", "link_new_window"], function() {
                            opts.next_query.opts[this] = opts[this]
                        })
                    }
                    $.post(opts.ajaxurl, opts.next_query).success(function(response) {
                        if ("-1" == response) {
