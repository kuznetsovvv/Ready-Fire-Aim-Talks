...
class WPTiles extends Abstracts\WPSingleton
{
    ...
    public function get_tiles( $posts = array(), $opts = array() ) {
        ...
        if ( is_array( $posts ) && ( empty( $posts ) || count( array_filter( array_keys( $posts ), 'is_string') ) ) ) {
            /*$posts['meta_key'] = 'unique_viewcount';
            $posts['orderby'] = 'meta_value_num';
            $posts['order'] = 'DESC';//we're going to comment this block out to just filter by newest-first */
            //echo("<pre>");
            //print_r($posts);
            //echo("</pre>");
            ...
            /*if(get_current_user_id() === 1){                                                //>Be me.
                $maxDate = 0;
                $secondMaxDate = 0;
                $tosort = $posts->posts;
                foreach($tosort as $postKey => $aPost){
                    $thisDate = get_the_time( "U", $aPost->ID );
                    if($thisDate > $secondMaxDate){
                        if($thisDate > $maxDate){
                            $secondMaxDate = $maxDate;
                            $maxDate = $thisDate;
                        }else{
                            $secondMaxDate = $thisDate;
                        }
                        if($maxDate - $secondMaxDate > 604800 ){
                            echo("it has been over a week");
                        }
                    }
                }
